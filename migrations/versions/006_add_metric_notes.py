...
Written by `alembic revision --autogenerate` from the model change, which is what it is for.
(001-003 were autogenerated too — they still carry the `# ### commands auto generated by Alembic`
banner. The hand-written outliers are 004 and 005, whose raw view SQL and `ALTER TYPE` autogenerate
cannot produce.)

It also correctly proposed NOTHING for the candidate_summary view, confirming the design claim in
`app/models/views.py`'s module docstring: keeping that module out of `migrations/env.py`'s imports
stops autogenerate from emitting CREATE/DROP TABLE for what is really a view.
"""

...
| [`type-safety-plan.md`](type-safety-plan.md) | **PR #15, shipped and merged 2026-08-25.** Making the domain vocabulary un-mistypeable: real enums for the 11 closed-set strings, `NewType` for the 49 identifiers, and the five stages it lands in. Carries the facts that took measuring — why moving `VariantKind` needs no migration, why `NewType` cannot cross into Strawberry. All five stages shipped. Two of its stated facts were wrong and are corrected in place. |
| [`scoreentry-plan.md`](scoreentry-plan.md) | **PR #16, shipped and merged 2026-09-19.** The ScoreEntry chapter: what is still unbuilt in the GraphQL layer, the order it has to land in, and why `Context.interface_kinds()` has to come first. Names what PR #15 handed over and did not close. All five stages shipped; the doc records what each stage's decisions turned out to be, including the 2/3 swap. One follow-up is open: issue #17, whether a request should be one snapshot. |
| [`bundle-at-the-boundary.md`](bundle-at-the-boundary.md) | **A good idea, agreed and not done.** Get the catalog's identity fields into a responsible object as soon as they leave SQL, rather than three call sites later. Carries the census of all five places they are created, the two that still produce raw tuples, and why `Heading`/`MetricIdentity`/`ScoreKey`/`HeadingAudit` are not one type despite sharing fields — which is how the drift started. |
| [`test-review-plan.md`](test-review-plan.md) | **A collecting doc, not a plan.** Items for the test-review pass that has been referenced since PR #13 without having a place to live. The theme added 2026-09-05: where a test reaches into an implementation, ask whether dependency injection would let it come through the front door — because a misaimed patch does not error, it passes, against the bug it was written to catch. Includes the `monkeypatch` inventory and the sites judged correct as they stand. |
...
| [`pr-15-diary.md`](pr-15-diary.md) | A narrative of the `type-safety` branch — twelve acts, written as each stage landed rather than after, and three review passes. The useful half is the corrections: two plan facts that were wrong, a stage reported as done that was not, and the same mistake (hand-listing what the code already knows) made three times. Primary source for the development blog, with PR #13's and #14's. |
| [`pr-16-diary.md`](pr-16-diary.md) | A narrative of the `score-entry-resolvers` branch — seven acts, the chapter where the API finally serves the 2026-07-31 finding. The useful half is one failure mode met five times: a check that passes while covering less than it claims (a mutation harness reading the wrong stream, a `-k` selecting two tests, `get_args` on a promoted NamedTuple running zero cases, four error-masking tests exercising an object that cannot occur, Alembic silencing the logger a test was watching). Also records a premise that was flatly wrong for a whole stage, and how running against the real corpus caught it. Primary source for the development blog, with #13, #14 and #15's. |

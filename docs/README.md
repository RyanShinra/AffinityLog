...
| [`scoreentry-plan.md`](scoreentry-plan.md) | **The plan for PR #16.** The ScoreEntry chapter: what is still unbuilt in the GraphQL layer, the order it has to land in, and why `Context.interface_kinds()` has to come first. Names what PR #15 handed over and did not close. **In progress on `score-entry-resolvers`:** stages 1, 1b and 3 shipped, stage 2 next; the doc records what each stage's decisions turned out to be, including the 2/3 swap. |
